...
HTTP download format identifiers keep the `_zip` suffix for multi-file formats because the browser receives one downloadable artifact. The [Go CLI export runner](../batch-component/go-cli-export-runner.md) uses unpacked format identifiers `csv`, `excel-csv`, `json`, `yaml`, and `ndjson` and writes those formats to an output directory instead of creating ZIP archives. Template-based source generation uses `masterdatamate generate`, not an export format.
...
エクスポートでは対象世代を明示指定でき、SPA のデフォルトは画面上の現在の出力対象選択を使う。GUI は形式ごとのエクスポート設定を読み込み、操作した日時形式や timezone などを `masterdata/export_settings.yaml` に保存して次回の初期値にする。Go CLI では `--generations` が省略された場合に generation 設定の `output: true` 世代を使い、`--time-format` や `--timezone` などが省略された場合は保存済みの形式別設定を使う。成果物作成前に、選択世代で統合した有効データだけを対象に厳密な事前チェックを行う。外部参照は保存済み主キーで検証し、参照先が選択世代の有効なエクスポート対象データに存在しない場合はエラーとして export を止める。標準CSVはUTF-8 BOMなし/LF/ヘッダーあり/RFC 4180相当で boolean は `true/false`、Excel CSVはBOMつきで boolean は `TRUE/FALSE` かつExcel向けの数式安全化を行う。日時は既定 `iso`、オプションで `epoch-sec`、`epoch-ms`、`iso-local` を選べる。出力形式は CSV ZIP、Excel CSV (BOM) ZIP、JSON ZIP、YAML ZIP、SQL、Excel workbook、SQLite、NDJSON ZIP を候補とする。Pongo2 テンプレートによるソース生成は `masterdatamate generate` が扱う。
