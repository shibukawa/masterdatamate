...
The AI provider configuration model defines how MasterDataMate connects its in-app AI assistant and agent tools to hosted or local LLM backends. It separates two runtime modes: a MasterDataMate-managed chat agent backed by API-level model providers, and delegated external agents such as Codex CLI that own their own planning, context management, and session behavior.

The managed chat agent supports OpenAI-compatible chat APIs as the primary transport shape, local API providers such as Ollama and LM Studio, and local API bridges such as Apple's `fm serve`. Delegated external agents are integrated as separate assistant runtimes, not as interchangeable low-level chat-completion providers.
...
| profiles.runtime_mode | enum | yes | `managed_chat_agent` for MasterDataMate-owned session/context/tool orchestration, or `delegated_external_agent` for agent runtimes such as Codex CLI that own their own context and task loop. |
...
`foundation_models_cli` may be used in direct command mode through commands such as `fm respond`, or in API bridge mode by connecting to an already-running `fm serve` process and treating its local Chat Completions API server as an `openai_compatible` profile. Direct command mode avoids a long-running server process. API bridge mode lets the host reuse the OpenAI-compatible adapter, but the initial implementation assumes the server lifecycle is user-managed and verifies readiness through health checks.

Unix domain socket bridge mode for `fm serve` is deferred until the installed command behavior and provider compatibility are proven reliable. The initial bridge shape is a user-managed loopback TCP endpoint such as `http://127.0.0.1:1976`. Loopback TCP keeps setup inspectable for manual debugging with tools such as mitmproxy and avoids host-owned process lifecycle issues.
...
- Foundation Models `fm serve` bridge mode must use a loopback endpoint by default. Binding to `0.0.0.0` or another non-loopback interface requires explicit user configuration.
...
- The initial implementation must not start `fm serve` from the Go host. Users start or manage the local server outside MasterDataMate.
- Host-managed `fm serve` process lifecycle, `transport: unix_socket`, and host-generated `unix_socket_path` are deferred provider features.

## Runtime Mode Rules

`managed_chat_agent` is the ordinary in-app assistant runtime. MasterDataMate owns the user-visible sessions, prompt-context message list, automatic compaction, tool registry, confirmation tokens, provider calls, and AG-UI event stream. Profiles in this mode should be API-level model providers or API-compatible local bridges such as OpenAI, OpenAI-compatible HTTP, Ollama, LM Studio, or `fm serve`.

`delegated_external_agent` is for existing agent runtimes that already implement their own planning loop, context compaction, file inspection, and task execution semantics. Codex CLI belongs in this mode. MasterDataMate may launch the external agent with scoped task instructions and workspace constraints, then render returned events or final results, but it must not pretend that the external agent is just another model behind the same prompt-context pipeline.

- Managed chat profiles use [AI assistant session model](ai-assistant-session-model.md) prompt context and compaction.
- Delegated external agent profiles may keep a MasterDataMate display session for UI continuity, but the external agent owns its internal session, prompt history, context compaction, and tool loop.
- The UI must expose the runtime mode clearly enough that users understand whether they are talking to the lightweight in-app assistant or handing off to a heavier external agent.
- Runtime switching is explicit. A session created for the managed chat agent is not silently continued inside Codex CLI, and a Codex CLI task is not silently replayed as managed chat history.
- Delegated external agents may be offered for heavier repository-aware tasks such as spec edits, code changes, and multi-step diagnosis. They should not be the default path for quick table, generation, or validation questions when a managed chat provider is available.
...
- Managed command-backed provider outputs must be normalized into the same internal assistant message, tool-call, tool-result, and usage shape when that metadata is available. Delegated external agent outputs are normalized into agent task events and results, not into the managed chat prompt-context message list.
...
profiles:
  ...
  - id: local-codex
    ...
    runtime_mode: delegated_external_agent
    command: codex
    args: [exec, --json]
    stdin_mode: prompt
    stdout_format: jsonl
...
OpenAI互換API、OpenAI公式API、Ollama、LM Studio、`fm serve` などは MasterDataMate がセッション、コンテキスト、コンパクト化、ツール実行を管理する `managed_chat_agent` として扱う。Codex CLI のような既存エージェントは `delegated_external_agent` として分け、Codex側のセッション管理・コンパクト化・タスクループに任せる。初期実装では無料で利用できるローカル `fm serve` をOpenAI互換の自前エージェント用プロファイルとして優先対応する。
